...
def main(config = {}, path_config ={}, test_epochs=10, dataset_select =None, tfBoard_enable=None, device = None):
    ...
    try:
        ...
        for epoch in range(TRAIN_epochs):
            ...
            loss_,lossbpr_,training_information = Procedure.DCC_train_original(dataset, encoder, dcc, epoch,
                                                                tf_writer=tf_writer)
            if epoch%10==0:
                epochs_list.append(epoch)
                loss_list.append(loss_)
                bprloss_list.append(lossbpr_)
            train_info = f'EPOCH[{epoch + 1}/{TRAIN_epochs}] {training_information}'
            print(train_info)
            # print(encoder.propagation_weight[0])
            # print(encoder.embedding_user.weight)
            train_infos.append(train_info+'\n')
            torch.save(encoder.state_dict(), weight_file)

    finally:
        plt.figure(1)
        plt.plot(epochs_list, loss_list, c='blue')
        plt.scatter(epochs_list, loss_list, c='blue')
        plt.xlabel("epochs", fontdict={'size': 16})
        plt.ylabel("loss", fontdict={'size': 16})
        # plt.figure(2)
        # plt.plot(epochs_list, bprloss_list, c='blue')
        # plt.scatter(epochs_list, bprloss_list, c='blue')
        # plt.xlabel("epochs", fontdict={'size': 16})
        # plt.ylabel("bpr_loss", fontdict={'size': 16})
        plt.show()    
        if tfBoard_enable:
            tf_writer.close()
...
def testResultStore(dataset, root_path, test_epochs, config, result, time_total, train_info):
    ...
    fileName =  dataName+ '_' + model + '_' + '.txt'
...
if __name__ == '__main__':
    ...
    config = {
        ...
        'multicore': 0,
        'lr': 2e-3,  # the learning rate: try 0.001
