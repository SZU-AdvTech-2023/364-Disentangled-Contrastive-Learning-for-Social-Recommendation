...
class Model(BasicModel):
    ...
    def __init_weight(self):
        ...
        self.Graph_= self.dataset.getSparseGraph_social()
    ...
    def propagation_(self, users_id, items_id, g_dropped, s_dropped):
        users_emb = users_id
        items_emb = items_id
        u_emb=torch.cat([users_emb,users_emb])
        ...
        # print('COMB权重值')
        # print(self.Graph_Comb.att_x.weight)
        embs = [all_emb]
        for layer in range(self.n_layers):
            ...
            users_emb_social = torch.sparse.mm(S, u_emb)
            # user-item bi-network propagation(user and item embedding)
            all_emb_interaction = torch.sparse.mm(A, all_emb)
            users1, users2 = torch.split(users_emb_social, [self.num_users, self.num_users])
            ...
            # graph fusion model
            users_emb_next = self.Graph_Comb(users1, users_emb_interaction)
    ...
    def computer(self):
        ...
        if self.config['dropout']:
            ...
            g_dropped = self.Graph
        g_d_=self.Graph_
        # users, items = self.propagation_(self.embedding_user.weight, self.embedding_item.weight, g_dropped, g_d_)
        users, items = self.propagation(self.embedding_user.weight, self.embedding_item.weight, g_dropped, None)
